...
class TenMicronMountFake:
    ...
    async def set_target_ra_dec(self, ra: Union[str, float], dec: Union[str, float]):
    ...
    async def set_target_alt_az(self, alt: Union[str, float], az: Union[str, float]):
    ...
    async def slew_to_target_equatorial(self, pier_side: Optional[str] = None) -> str:
    ...
    async def send_command(self, cmd: str, *args, **kwargs) -> str:
...
async def main():
    ...
    await mount.get_status()
    ra, dec = await mount.get_mount_ra_dec()
    print("RA:", ra, "DEC:", dec)
    alt, az = await mount.get_mount_alt_az()
    print("ALT:", alt, "AZ:", az)
    await mount.set_target_ra_dec("12:34:56.78", "+22:33:44.5")
    print(await mount.slew_to_target_equatorial())
    print(await mount.get_status())
    ...
    print("Pier Side:", await mount.pier_side())
    await mount.flip()
    print("Pier Side after flip:", await mount.pier_side())

    await mount.park()
    print("Status after park:", await mount.get_status())
    await mount.unpark()
    print("Status after unpark:", await mount.get_status())

    print("Firmware Date:", await mount.firmware_date())
    print("Product Name:", await mount.product_name())
    print("Element 1 Temp:", await mount.get_element_temperature(1))
